...
class _DevicesScreenState extends State<DevicesScreen> with ResponsiveMixin {
  ...
  void didChangeDependencies() {
    ...
    if (!_isInitialized) {
      _isInitialized = true;
      _loadDevices();
      print('🚀 DevicesScreen: Initial data load triggered');
    } else {
      print(
        '📱 DevicesScreen: Dependencies changed (likely screen resize) - NO API call',
      );
    }
  }
  ...
  void handleResponsiveStateChange() {
    ...
    if (isMobile && !_isKanbanView) {
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        if (mounted) {
          setState(() {
            _isKanbanView = true;
            _currentViewMode = DeviceDisplayMode.kanban;
          });
        }
      });
    }
    ...
    print(
      '📱 DevicesScreen: Responsive state updated (mobile: $isMobile, kanban: $_isKanbanView, view: $_currentViewMode) - UI ONLY, no API calls',
    );
  }
  ...
  void _onSearchChanged(String value) {
    ...
    _searchTimer = Timer(const Duration(milliseconds: 500), () {
      if (mounted) {
        ...
        print('🔍 DevicesScreen: Search triggered for: "$value"');
        _loadDevices(); // Trigger API call after delay
      }
    });
  }
              ...
              body: SafeArea(
                child: Column(
                  children: [
                    ...
                    // Pagination only when not in map view
                    if (_currentViewMode != DeviceDisplayMode.map)
                      _buildPagination(),
                  ],
                ),
              ),
        ...
        onSelected: (value) {
          switch (value) {
            ...
            case 'refresh':
              ...
              break;
            case 'kanban':
              _onViewModeChanged(DeviceDisplayMode.kanban);
              break;
            case 'table':
              _onViewModeChanged(DeviceDisplayMode.table);
              break;
            case 'map':
              _onViewModeChanged(DeviceDisplayMode.map);
              break;
          }
        },
        itemBuilder: (context) => [
  ...
  void _onViewModeChanged(DeviceDisplayMode mode) {
    setState(() {
      ...
      _isKanbanView = (mode == DeviceDisplayMode.kanban);
    ...
    print(
      '🔄 DevicesScreen: View mode changed to $mode (kanban: $_isKanbanView)',
    );
  }
  ...
  Widget _buildContent() {
    ...
    // Show error state if there's an error and no devices
